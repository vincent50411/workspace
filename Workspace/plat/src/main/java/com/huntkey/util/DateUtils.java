...
public class DateUtils
{
     ...
     * @return
     */
    public static String getQrtByMonth(Object str)
    {
        String monthStr = formatDate(parseDate(str), "MM");

        int monthValue = Integer.valueOf(monthStr);

        String months[] = { "01", "04", "07", "10" };
        if (monthValue >= 1 && monthValue <= 3) // 1-3月;
            return months[0];
        else if (monthValue >= 4 && monthValue <= 6) // 4-6月;
            return months[1];
        else if (monthValue >= 7 && monthValue <= 9) // 7-9月;
            return months[2];
        else
            // 10-12月;
            return months[3];
    ...
    /**
     * 获取开始时间和结束时间为区间内的随机日期，时间格式为
     * @param start 支持的日期字符串格式包括"yyyy-MM-dd","yyyy-MM-dd HH:mm:ss", "yyyy-MM-dd HH:mm", "yyyy/MM/dd", "yyyy/MM/dd HH:mm:ss", "yyyy/MM/dd HH:mm"
     * @param end   支持的日期字符串格式包括"yyyy-MM-dd","yyyy-MM-dd HH:mm:ss", "yyyy-MM-dd HH:mm", "yyyy/MM/dd", "yyyy/MM/dd HH:mm:ss", "yyyy/MM/dd HH:mm"
     * @param pattern   时间格式，例如"yyyy-MM-dd"，"yyyy-MM-dd HH:mm:ss"等
     * @return
     */
    public static String getDateStrFromSegment(Object start, Object end, String pattern)
    {
        ...
        long min = sDate.getTime();

        long max = eDate.getTime();
        ...
        return DateUtils.getDate(calendar.getTime(), pattern);
    }
     ...
     * @return
     */
    public static Date getDateFromSegment(Object start, Object end, String pattern)
    {
        return parseDate(getDateStrFromSegment(start, end, pattern));
    }
